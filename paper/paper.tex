...
this paper, we address a new important problem of maximizing the number of
completed workflows from an ensemble under both budget and deadline constraints
on the Infrastructure-as-a-Service (IaaS) clouds, characterized by dynamic
resource provisioning capabilities and pay-per-use model.  We discuss, develop
and assess algorithms based on static and dynamic strategies for both task
scheduling and resource provisioning. We evaluate the algorithms via simulation
using a set of scientific workflow ensembles with a broad range of budget and
deadline parameters. This allows to identify the application scenarios and
constraint ranges for which the algorithms perform better. We found that the key
for algorithm performance is the decision which workflows should be admitted or
rejected from the ensemble. The results show that the admission procedure based
on worklfow structure and estimates of task runtimes can significantly improve
...
\begin{itemize}
  \item definition of the scheduling problem of priorotized workflows grouped in
  ensembles, under both budget and deadline constraints,
  \item analysis of static and dynamic strategies for task scheduling and
  resource provisioning on clouds, together with the admission procedure to
  predict which workflows from the ensemble can be completed,
  \item development of static and dynamic algorithms and of the simulator based
  on the defined application and infrastructure model,
  \item performance evaluation of algorithms on the set of synthetic benchmark
  workflow ensembles representing important real scientific applications,
  \item analysis of results, which allows to assess the advantages and
  disadvantages of proposed algorithms, depending on application scenarios and
