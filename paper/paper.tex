...
is that the number of resources can be changed in response to application demand. 
In policy-based provisioning (or auto-scaling) systems, the number of provisioned VMs 
is adjusted based on application demand as measured by metrics such as resource 
utilization and queue length. The DPDS algorithm uses resource utilization
to determine when to provision and de-provision VMs. The provisioner
...
already paid for are not terminated prematurely the \textsc{Terminate} procedure
...
observation leads to the improved algorithm which we call Workflow-Aware DPDS
(WA-DPDS).
