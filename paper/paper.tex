...
\address[agh]{AGH University of Science and Technology, Dept. of Computer Science, Krakow, Poland} 
...
of results based on deadline and budget constraints. We also report on new
results and discussion of task granularity in the context of the resource
billing cycle, which is important aspect of scheduling and resource provisioning
...
different from \cite{Yu2005} and \cite{Abrishami2013158} in that we consider ensembles of workflows in
...
\subsection{Results vs. deadline and budget}
...
on 100 different simulation runs. In Fig.~\ref{fig:completed-deadlines}
...
Fig.~\ref{fig:completed-deadlines} shows the results depending on normalized
...
(sorted). This results from the fact that in the unsoted distributions the large
...
One interesting observation from Fig.~\ref{fig:completed-deadlines} is that for
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\columnwidth]{run-stretching-test-output-scaling-CYBERSHAKE}
    \includegraphics[width=0.9\columnwidth]{run-stretching-test-output-scaling-MONTAGE}
    
    \caption{Percentage of high scores achieved by each algorithm for CyberShake
    and Montage ensembles when task runtime is scaled by a constant factor. A
...
Fig.~\ref{fig:stretching} shows the relative performance of the algorithms as
...
calculated the same way as it was for Fig.~\ref{fig:distributions}.
...
The granularity of the workflow tasks should be discussed in comparison with the
length of the cloud billing cycle. In this paper we assumed that the resources
are charged for hourly time slots, as is the case of Amazon EC2. However, there
are cloud providers that have higher billing frequency, e.g. every 5 minutes in
the case of CloudSigma\footnote{\url{http://www.cloudsigma.com}} or every 1
minute in the case of Google Compute
...
better. This suggests that the these effects have to be taken into account when
