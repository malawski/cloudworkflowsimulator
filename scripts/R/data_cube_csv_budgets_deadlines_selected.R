...
pdf(file=paste(csv_filename, "-deadlines-app-dist-selected.pdf", sep=""), height=6, width=13, bg="white")
par(mfrow=c(2,5))
...
pdf(file=paste(csv_filename, "-budgets-app-dist-selected.pdf", sep=""), height=6, width=13, bg="white")
par(mfrow=c(2,5))
#select only cybershake and ligo
for (app in c(1,3)) {
	for (j in 1:ndistributions) {
		...
		plot(xrange, yrange, type="n", xlab="Normalized budget",
				ylab="Completed Workflows", main = paste(c(applications[app],distributions[j])))
		legend("topleft", c(algorithms[1:length(algorithms)]), cex=1, col=2:(length(algorithms)+1), pch=1:length(algorithms) )
