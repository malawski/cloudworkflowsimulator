...
csv_filename = "run-finish-delays-test-0-output.dat"
...
cube_dimensions = c(n_algorithms, n_runIds, n_deadlines, n_budgets, n_distributions, n_applications, n_delays)
scoreBitString = data[order(data$delay, data$application, data$distribution, data$budget, data$deadline, data$seed, data$algorithm),]$scorebits
...
pdf(file=paste(csv_filename, "-distributions-delays.pdf", sep=""), height=2, width=7, bg="white")
par(mfrow=c(1,3))
par(mar=c(4,4,2,0))
...
for (i in c(1,3)) {
	...
	colnames(bar_data) = delays
	rownames(bar_data) = algorithms
	# reorder algorithms and change to %
	bar_data = bar_data[c(1,3,2),]/50.0
	print(bar_data)	
	barplot(as.table(bar_data), beside = TRUE, xlab="Provisioning delay (s)",  main = applications[i], 
