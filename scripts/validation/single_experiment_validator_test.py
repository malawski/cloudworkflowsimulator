import unittest
from scripts.validation import single_experiment_validator
from scripts.validation.parsed_log_loader import TaskLog
from scripts.validation.parsed_log_loader import TransferLog
from scripts.validation.parsed_log_loader import VMLog
...
class SingleTaskValidatorTest(unittest.TestCase):
    ...
    def test_should_return_some_message_when_fails(self):
        task = TaskLog(started=single_experiment_validator.MISSING_VALUE,
                       finished=single_experiment_validator.MISSING_VALUE,
                       **IRRELEVANT_TASK_ATTRIBUTES)
    ...
    def test_should_fail_when_task_has_not_ended(self):
        task = TaskLog(finished=single_experiment_validator.MISSING_VALUE,
    ...
    def test_should_pass_when_valid_transfer(self):
        ...
        result = single_experiment_validator.validate_transfer(task)

        self.assertTrue(result.is_valid)

    def test_should_return_some_message_when_transfer_validation_fails(self):
        task = TransferLog(started=single_experiment_validator.MISSING_VALUE,
                           finished=single_experiment_validator.MISSING_VALUE,
                           **IRRELEVANT_TRANSFER_ATTRIBUTES)
    ...
    def test_should_fail_when_transfer_has_not_ended(self):
        task = TransferLog(finished=single_experiment_validator.MISSING_VALUE,
    ...
    def test_should_pass_when_valid_vm(self):
        ...
        result = single_experiment_validator.validate_vm(task)

        self.assertTrue(result.is_valid)

    def test_should_return_some_message_when_vm_validation_fails(self):
        task = VMLog(started=single_experiment_validator.MISSING_VALUE,
                     finished=single_experiment_validator.MISSING_VALUE,
                     **IRRELEVANT_VM_ATTRIBUTES)
    ...
    def test_should_fail_when_vm_has_not_ended(self):
        task = VMLog(finished=single_experiment_validator.MISSING_VALUE,
