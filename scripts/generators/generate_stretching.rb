...
prefix = "stretching-test"
applications = ["CYBERSHAKE", "MONTAGE"]
    ...
    for application in applications do
      for algorithm in algorithms do
        for distribution in distributions do
          ...
          task_id = task_id + 1
          id_string = "%08d" % id
          args = "-application #{application} -inputdir ../projects/pegasus/#{input_dirs[application]}/ -outputfile #{run_dir}/#{prefix}-output-#{id_string}.dat -distribution #{distribution} -ensembleSize #{ensemble_size} -algorithm #{algorithm} -seed #{seed} -failureRate #{failure_rate} -runtimeVariance #{variation} -delay #{delay} -scalingFactor #{scaling_factor}\n"
          f.write args
        end
      end  
    end
