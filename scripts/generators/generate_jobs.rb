...
OptionParser.new do |opts|
  ...
  opts.on("--dag-prefix PREFIX", String, "Required dag path prefix. This is the folder with " +
  ...
  opts.on("--out-prefix PREFIX", String, "Required output path prefix. This is the location " +
  "that output files of this script will be located. On grid environments this should be " +
  "on fast-access filesystem.") do |val|
    out_prefix = val
  end
  opts.on("--name-prefix PREFIX", String, "Optional input files' prefix, defaults to " +
  "\"#{prefix}\". All input and output files of simulations will be prefixed with " +
  ...
  opts.on("-l", "--[no-]local", "Generate commands for running simulations locally, defaults to #{local}") do |v|
    local = v
  end
  opts.on("-r", "--[no-]run", "Run generated commands instead of printing them, defaults to #{run}") do |v|
    run = v
  end
