...
csv_filename = "run-finish-variations-test-0-output"
...
par(mfrow=c(1,7))
for (runtimeVariance in runtimeVariances) {
	print(runtimeVariances)
	boxplot(jobfinish/deadline~algorithm, las = 2, data = data[data$runtimeVariance==runtimeVariance,],main = sprintf("%2.0f %%",runtimeVariance*100),cex.names=0.8,ylim = c(0,2),ylab = "jobfinish / deadline")
	abline(h = 1, col = "red")
}
dev.off()
...
for (runtimeVariance in runtimeVariances) {
	print(runtimeVariances)
	boxplot(dagfinish/deadline~algorithm, las = 2, data = data[data$runtimeVariance==runtimeVariance,],main = sprintf("%2.0f %%",runtimeVariance*100),cex.names=0.8,ylim = c(0,2),ylab = "dagfinish / deadline")
	abline(h = 1, col = "red")
}
dev.off()
...
for (runtimeVariance in runtimeVariances) {
	print(runtimeVariances)
	boxplot(vmfinish/deadline~algorithm, las = 2, data = data[data$runtimeVariance==runtimeVariance,],main = sprintf("%2.0f %%",runtimeVariance*100),cex.names=0.8,ylim = c(0,2),ylab = "vmfinish / deadline")
	abline(h = 1, col = "red")
}
dev.off()
...
for (runtimeVariance in runtimeVariances) {
	print(runtimeVariances)
	boxplot((vmfinish-deadline)/3600.0~algorithm, las = 2, data = data[data$runtimeVariance==runtimeVariance,],main = sprintf("%2.0f %%",runtimeVariance*100),cex.names=0.8,ylim = c(-4,4),ylab = "vmfinish - deadline in hours")
	abline(h = 1, col = "red")
}
dev.off()
...
par(mfrow=c(1,7))
for (runtimeVariance in runtimeVariances) {
	print(runtimeVariance)
	boxplot(cost/budget~algorithm, las = 2, data = data[data$runtimeVariance==runtimeVariance,],main = sprintf("%2.0f %%",runtimeVariance*100),cex.names=0.8,ylim = c(0,2),ylab = "cost / budget")
	abline(h = 1, col = "red")
}
dev.off()

pdf(file=paste(csv_filename, "-cost-spss.pdf", sep=""), height=4, width=4, bg="white")
boxplot(cost/budget~runtimeVariance, las = 2, data = data[data$algorithm=="SPSS",],
		main = sprintf("SPSS"),cex.names=0.8,ylim = c(0,2),
		ylab = "cost / budget",xlab = "runtime variance", 
		names=sprintf("%1.0f%%", runtimeVariances*100))
abline(h = 1, col = "red")
dev.off()
...
for (distribution in distributions) {
	for (application in applications) {
		boxplot(cost/budget~algorithm:runtimeVariance, border = rainbow(3), col = "lightgrey", 
...
for (distribution in distributions) {
	for (application in applications) {
		boxplot(dagfinish/deadline~algorithm:runtimeVariance, border = rainbow(3), col = "lightgrey", 
				las = 2, data = data[data$application==application & data$distribution==distribution,],
				main = sprintf("%s, %s",application, distribution),cex.names=0.8, 
...
par(mar=c(4.5,4,0.1,0.01), oma=c(1,0,0,0),xpd=T)
boxplot(cost/budget~algorithm:runtimeVariance, col=gray( c(0.3,0.6,0.9) ), 
		las = 2, data = data,
		xaxt='n',
		#outline = FALSE,
...
axis(1, 0:6*3+2.0, tck=0, sprintf("%2.0f %%",runtimeVariances*100), cex.axis = 1, lty=0, pos=-0.27) 
legend(0, 2.05, algorithms, fill = gray( c(0.3,0.6,0.9)), cex=0.75)
#par(xpd=T)
text(11,-0.54,"Runtime estimate error")
par(xpd=F)
abline(h = 1, col = "red")
dev.off()
...
par(mar=c(4.5,4,0.1,0.01), oma=c(1,0,0,0),xpd=T)
boxplot(dagfinish/deadline~algorithm:runtimeVariance, col=gray( c(0.3,0.6,0.9) ), 
		las = 2, data = data,
		xaxt='n',
#		outline = FALSE,
		range = 0, # whiskers exend to the max
