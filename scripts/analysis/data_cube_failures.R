...
n_failureRates = length(failureRates)
...
cube_dimensions = c(n_algorithms, n_runIds, n_deadlines, n_budgets, n_distributions, n_applications, n_failureRates)
scoreBitString = data[order(data$failureRate, data$application, data$distribution, data$budget, data$deadline, data$seed, data$algorithm),]$scorebits
...
# sum scores, grouping by algorithm, application and delay
sumScores = apply(topScores,c(1,6,7),sum)



pdf(file=paste(csv_filename, "-distributions-failures.pdf", sep=""), height=2, width=7, bg="white")
par(mfrow=c(1,3))
par(mar=c(4,4,2,0))
...
for (i in c(1,3)) {
	...
	colnames(bar_data) = failureRates*100
	rownames(bar_data) = algorithms
	# reorder algorithms and change to %
	bar_data = bar_data[c(1,3,2),]/50.0
	print(bar_data)	
	barplot(as.table(bar_data), beside = TRUE, xlab="Failure rate (%)",  main = applications[i], 
