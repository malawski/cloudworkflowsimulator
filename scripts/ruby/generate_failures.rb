...
failure_rates = [0.00, 0.01, 0.02, 0.05, 0.10, 0.20, 0.50]
ensemble_size = 50
id = 0
    ...
    for application in applications do
      for algorithm in algorithms do
        for distribution in distributions do
          ...
          task_id = task_id + 1
          id_string = "%08d" % id
          args = "-application #{application} -inputdir ../projects/pegasus/#{input_dirs[application]}/ -outputfile #{run_dir}/#{prefix}-output-#{id_string}.dat -distribution #{distribution} -ensembleSize #{ensemble_size} -algorithm #{algorithm} -seed #{seed} -failureRate #{failure_rate} \n"
          f.write args
        end
      end  
    end
